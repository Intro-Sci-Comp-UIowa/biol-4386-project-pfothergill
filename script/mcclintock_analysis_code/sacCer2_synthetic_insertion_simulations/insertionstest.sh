...
rundir=$1
output_dir=$1"/singleinsertionsims"
mkdir -p $output_dir
...
TElines=("" "81,199" "509,627" "401,507" "201,290")
...
do
	...
	cat <(sed -n "$id,$start p" $output_dir/data/fragments.fasta) <(sed -n "${TElines[$TE]} p" $rundir/mcclintock/test/sac_cer_TE_seqs.fasta)  <(sed -n "$end p" $output_dir/data/fragments.fasta) > $output_dir/data/$i".fasta"
...
echo "bash launchmcclintock.sh 1 $rundir" | qsub -V -cwd -l cores=4 -l long -N insertion_1 -o $output_dir"/qsub_output/insertion_1.o" -e $output_dir"/qsub_output/insertion_1.e"

for i in {2..299}
do
	echo "bash launchmcclintock.sh $i $rundir" | qsub -V -cwd -l cores=4 -l long -N insertion_$i -hold_jid insertion_1 -o $output_dir"/qsub_output/insertion_"$i".o" -e $output_dir"/qsub_output/insertion_"$i".e"
done
